...
void Consola::clrscr() {
  ...
  FillConsoleOutputCharacter(hconsola, ' ', csbi.dwSize.X * csbi.dwSize.Y,
                             startCoords, &dummy);
  FillConsoleOutputAttribute(hconsola, csbi.wAttributes,
                             csbi.dwSize.X * csbi.dwSize.Y, startCoords,
                             &dummy);
...
void Consola::setScreenSize(int nLinhas, int nCols) {
  ...
  SetConsoleWindowInfo(
      hconsola, TRUE,
      &DisplayArea); // isto muda o tamanho da area da janela em caracteres
}
...
char Consola::getch(void) {
  ...
  while (ReadConsoleInputA(hStdin, &irInputRecord, 1,
    ...
    if (irInputRecord.EventType == KEY_EVENT &&
      ...
      ReadConsoleInputA(hStdin, &irInputRecord, 1,
      ...
      if (irInputRecord.Event.KeyEvent.wVirtualKeyCode == VK_RIGHT)
        return DIREITA;
      if (irInputRecord.Event.KeyEvent.wVirtualKeyCode == VK_UP)
        return CIMA;
      if (irInputRecord.Event.KeyEvent.wVirtualKeyCode == VK_DOWN)
        return BAIXO;
...
typedef DWORD(
    WINAPI *GetNumberOfConsoleFonts_)(); // kernel32!GetNumberOfConsoleFonts
...
void Consola::setTextSizeXP(int x, int y) {
  ...
  for (size_t i = 0; i < NumFonts; ++i) {
    // Indaga o tamanho (suportado) da fonte. Pode não haver nenhuma para esse
    // tamanho
    ConsoleInfo[i].dwFontSize = GetConsoleFontSize(
        GetStdHandle(STD_OUTPUT_HANDLE), ConsoleInfo[i].nFont);
